...
test_builds_deterministically() {
	fixture_build "$SNDOCS_TEST_TMP/a"
	fixture_build "$SNDOCS_TEST_TMP/b"

	for branch in australia zurich; do
		assert_equals \
			"$(git -C "$SNDOCS_TEST_TMP/a" rev-parse "$branch")" \
			"$(git -C "$SNDOCS_TEST_TMP/b" rev-parse "$branch")" \
			"two builds should produce the same commit on $branch"
	done
}

test_publishes_one_branch_per_family() {
	branches=$(git -C "$SNDOCS_TEST_FIXTURE" for-each-ref --format='%(refname:short)' refs/heads | LC_ALL=C sort | tr '\n' ' ')
	assert_equals 'australia zurich ' "$branches"
}

test_carries_a_topic_above_the_size_threshold() {
	path="$SNDOCS_TEST_FIXTURE/markdown/api-reference/c_GlideRecordAPI.md"
	size=$(wc -c <"$path" | tr -d ' ')
...
test_carries_an_empty_topic() {
	path="$SNDOCS_TEST_FIXTURE/markdown/administer/empty-topic.md"
...
test_omits_the_synonym_reference_from_the_repository_index() {
	assert_file_exists "$SNDOCS_TEST_FIXTURE/markdown/vocabulary/sn-docs-synonym-terms-enus.md"
	assert_not_contains "$(cat "$SNDOCS_TEST_FIXTURE/llms.txt")" 'synonym' \
		'the repository index should omit the synonym reference, as upstream does'
}

test_carries_realistic_frontmatter() {
	head=$(head -11 "$SNDOCS_TEST_FIXTURE/markdown/platform-security/acl-rules.md")
